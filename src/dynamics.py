...
class Quadrotor2D:
    ...
    def dynamics_true_no_disturbances(self, xk, uk, dt=None):
        """
        ...
        x_next = np.zeros(6)
        x_next[0] = x + dt*vx
        x_next[1] = vx + dt*((-(T1+T2)*np.sin(phi))/self.m) # - self.CD_v*vx + self.wx
        x_next[2] = y + dt*vy
        x_next[3] = vy + dt*(((T1+T2)*np.cos(phi))/self.m - self.g) # - self.CD_v*vy + self.wy
        x_next[4] = phi + dt*om
        x_next[5] = om + dt*((T2-T1)*self.l)/self.Iyy # - self.CD_phi*om
