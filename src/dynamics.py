...
def step_single_gpu(state, action, diameter, mass, Ix, Iy, Iz, g, thrust_coef, drag_yaw_coef, drag_force_coef, dt):
    """
    ...
    s_ψ, c_ψ,     = cp.sin(ψ), cp.cos(ψ)
    s_θ, c_θ, t_θ = cp.sin(θ), cp.cos(θ), cp.tan(θ)
    s_φ, c_φ,     = cp.sin(φ), cp.cos(φ)
